...
`define SDFFILE "../../dc/huffman_syn.sdf"  // Modify your sdf file name
...
`endif
  ...
  initial begin
    ...
    // if (CNT_valid)
    //  $display("%d %d %d %d %d %d", CNT[1], CNT[2], CNT[3], CNT[4], CNT[5], CNT[6],);
  end
  ...
  initial begin
    ...
    gray_data = pat_mem[i];
    for (i = 1; i < 100; i = i + 1) @(negedge CLK) gray_data = pat_mem[i];
      ...
      if (code_valid == 1'b1) begin
        ...
        case ({
          ...
          2'b11: begin
            $display("Check HC : PASS");
            $display("Check M : PASS");
            if (flag1 == 1'b1) $display("You PASS the contest now.");
            else $display("Where are those CNT output?");
            $finish;
            //$stop;
          end
        endcase
